...
### 1. clear:both
比如说，我们可能希望某个段落与浮动元素保持相邻的位置，但又希望这个段落从头开始强制独占一行，这时候我们可以使用 clear 属性。
clear 属性指定一个元素是否必须移动 (清除浮动后) 到在它`之前`的`浮动元素``下面`。clear 属性适用于浮动和非浮动元素。
1. 当应用于`非浮动块`时，它将非浮动块的边框边界移动到所有相关浮动元素外边界的下方。这个非浮动块的垂直外边距会折叠。
2. 当应用于`浮动元素`时，它将元素的外边界移动到所有相关的浮动元素外边框边界的下，两个浮动元素的垂直外边距将不会折叠。
```html
<div class="float"></div>

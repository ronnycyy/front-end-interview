...
### fAMD(实现AMD规范的require.js)  fCMD(实现CMD规范的sea.js，define语法，了解即可)  浏览器端
...
```
# f伪元素
伪元素是一个附加至选择器末的关键词，用于对元素的特定`部分`修改样式。 注意: `伪类`是修改元素特殊`状态`下的样式，这就是两者的区别。
```css
/* 
...
# fbefore fCSS伪元素
创建一个伪元素，其将成为匹配元素的第一个子元素。常通过 content 属性来为一个元素添加修饰性的内容。此元素默认为行内元素。
## 注意
由::before 和::after 生成的伪元素 包含在元素格式框内， 因此不能应用在`f可替换元素`上， 比如<img>或<br> 元素。
## 例子: 加入引用标记
```html
...
# fafter fCSS伪元素
创建一个伪元素，作为已选中元素的最后一个子元素。通常会配合content属性来为该元素添加装饰内容。这个虚拟元素默认是行内元素。
```html
<!-- 这个段落最后会有红色的 "<- 无聊！" -->
<p class="boring-text">这是些无聊的文字</p>
<p>这是不无聊也不有趣的文字</p>
<!-- 这个段落最后会有绿色的 "<- 让人兴兴兴奋！" -->
<p class="exciting-text">在 MDN 上做贡献简单又轻松。按右上角的编辑按钮添加新示例或改进旧示例！</p>
```
```css
.boring-text::after {
  content: "<- 无聊！";
